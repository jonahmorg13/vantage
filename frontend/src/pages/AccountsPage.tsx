...
import { Modal, FormGroup, FormInput, FormSelect } from '../components/ui/Modal'
...
const DEFAULT_COLORS = [
  '#7c6dfa', '#fa6d8e', '#6dfab0', '#fac86d', '#6db8fa',
  '#fa9d6d', '#b06dfa', '#fa6dc8', '#6dfaed', '#faed6d',
]
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="font-sans text-3xl font-extrabold tracking-tight gradient-text">Accounts</h1>
          <p className="text-text3 text-sm mt-1">Track your checking, savings, and investment accounts</p>
        </div>
        <Button
          variant="primary"
          onClick={() => {
      ...
      {accounts.length > 0 && (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5 mb-6">
          <div className="bg-surface border border-border rounded-xl px-5 py-4">
            ...
            <div className={`font-mono text-2xl font-bold ${totalNetWorth < 0 ? 'text-danger' : 'text-accent3'}`}>
              {totalNetWorth < 0 ? '-' : ''}{formatCurrency(Math.abs(totalNetWorth))}
            </div>
          </div>
          <div className="bg-surface border border-border rounded-xl px-5 py-4">
          ...
          <div className="bg-surface border border-border rounded-xl px-5 py-4">
            <div className="text-xs text-text3 uppercase tracking-[0.1em] mb-1">Total Transactions</div>
              ...
              <Panel
                ...
                action={
                  <div className="flex items-center gap-2">
                    <Button variant="secondary" onClick={() => openEditAccount(account)} className="text-xs py-1.5 px-3">
                      Edit
                    </Button>
                    <Button variant="danger" onClick={() => deleteAccount(account.id)} className="text-xs py-1.5 px-3">
                      Delete
                    </Button>
                  </div>
                }>
                <div className="px-6 py-5">
                  ...
                  <div className="flex items-center gap-3 mb-5">
                    ...
                    <span
                      ...
                      style={{ color: account.color, borderColor: account.color + '40', background: account.color + '18' }}>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-5">
                    <div className="bg-surface2 rounded-lg px-4 py-3">
                      <div className="text-xs text-text3 uppercase tracking-[0.1em] mb-1">Initial Balance</div>
                      <div className="font-mono text-base font-bold text-text">{formatCurrency(account.initialBalance)}</div>
                    </div>
                    <div className="bg-surface2 rounded-lg px-4 py-3">
                      <div className="text-xs text-text3 uppercase tracking-[0.1em] mb-1">Current Balance</div>
                      <div className={`font-mono text-base font-bold ${balance < 0 ? 'text-danger' : 'text-accent3'}`}>
                        {balance < 0 ? '-' : ''}{formatCurrency(Math.abs(balance))}
                      </div>
                    </div>
                    <div className="bg-surface2 rounded-lg px-4 py-3">
                      <div className="text-xs text-text3 uppercase tracking-[0.1em] mb-1">Transactions</div>
                      <div className="font-mono text-base font-bold text-accent">{activity.length}</div>
                    </div>
                  </div>
                  ...
                  {activity.length > 0 && (
                    <div>
                      <div className="text-xs text-text3 uppercase tracking-[0.1em] mb-2">Recent Activity</div>
                      <div className="flex flex-col gap-1.5 max-h-48 overflow-y-auto">
                        {activity.slice(0, 20).map(tx => {
                          const isIncoming = (tx.type === 'income' && tx.accountId === account.id) ||
                            (tx.type === 'transfer' && tx.toAccountId === account.id)
                          const isOutgoing = (tx.type === 'expense' && tx.accountId === account.id) ||
                            (tx.type === 'transfer' && tx.accountId === account.id)
                          const amountColor = isIncoming ? 'text-accent3' : isOutgoing ? 'text-accent2' : 'text-text'
      ...
      <Modal
        ...
        <FormGroup label="Account Type">
          <FormSelect
            ...
            onChange={e => setAccountForm(f => ({ ...f, accountType: e.target.value as Account['accountType'] }))}>
        ...
        <div className="flex gap-3 mt-6 justify-end">
          <Button variant="secondary" onClick={closeAccountModal}>Cancel</Button>
