...
export function RecurringManager() {
  ...
  return (
    <>
      ...
      >
        <div>
          <p className="text-xs text-text3 px-6 py-4">
            Recurring transactions are auto-generated as pending entries when you navigate to a new month.
          </p>
            ...
            <table className="w-full border-collapse">
              <thead>
                <tr>
                  ...
                  <th className="px-6 py-3 text-left text-xs text-text3 tracking-[0.12em] uppercase border-b border-border">Amount</th>
                  <th className="px-6 py-3 text-left text-xs text-text3 tracking-[0.12em] uppercase border-b border-border">Type</th>
                  <th className="px-6 py-3 text-left text-xs text-text3 tracking-[0.12em] uppercase border-b border-border">Day</th>
                  <th className="px-6 py-3 text-left text-xs text-text3 tracking-[0.12em] uppercase border-b border-border">Active</th>
              ...
              <tbody>
                {state.recurringTransactions.map(r => (
                  <tr key={r.id} className="hover:bg-accent/[0.04]">
                    ...
                    <td className={`px-6 py-3 text-sm border-b border-white/[0.03] ${r.type === 'income' ? 'text-accent3' : 'text-accent2'}`}>
                      {r.type === 'income' ? '+' : ''}{formatCurrency(r.amount)}
                    </td>
                    <td className="px-6 py-3 text-sm text-text2 border-b border-white/[0.03] capitalize">{r.type}</td>
                    <td className="px-6 py-3 text-sm text-text2 border-b border-white/[0.03]">{r.dayOfMonth}</td>
                    <td className="px-6 py-3 border-b border-white/[0.03]">
                      <button
                        onClick={() => dispatch({ type: 'UPDATE_RECURRING', id: r.id, updates: { isActive: !r.isActive } })}
                    ...
                    <td className="px-6 py-3 border-b border-white/[0.03]">
                      <div className="flex gap-1.5">
                        ...
                        <button
                          onClick={() => dispatch({ type: 'DELETE_RECURRING', id: r.id })}
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editing ? 'Edit Recurring' : 'Add Recurring'}>
        <FormGroup label="Name / Description">
          <FormInput type="text" placeholder="e.g. Roth IRA" value={name} onChange={e => setName(e.target.value)} />
        </FormGroup>
        <div className="grid grid-cols-2 gap-4">
          <FormGroup label="Amount ($)">
            <FormInput type="number" placeholder="0.00" step="0.01" value={amount} onChange={e => setAmount(e.target.value)} />
          </FormGroup>
          <FormGroup label="Day of Month">
            <FormInput type="number" min="1" max="31" value={dayOfMonth} onChange={e => setDayOfMonth(e.target.value)} />
          </FormGroup>
        </div>
        <div className="grid grid-cols-2 gap-4">
        ...
        <div className="flex gap-3 mt-6">
          ...
          <Button onClick={handleSave} className="flex-1 !py-3">{editing ? 'Save Changes' : 'Add Recurring'}</Button>
        </div>
      </Modal>
    </>
  )
